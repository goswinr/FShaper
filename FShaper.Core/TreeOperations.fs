...
module ExprOps = 
    ...
    let sequentialToList sequential = 
        let rec loop acc s = 
            match s with 
            | Expr.Sequential (_, c, d) -> loop [] d @ loop [] c @ acc // NB: building the list backwards
            | x -> [x]
...
module ParserUtil = 
    ...
    let rec replaceExpr f tree = 
        ...
        | None -> 
            match tree with 
            | Expr.Sequential (s2,s3,s4) -> Expr.Sequential (s2, replaceExpr s3, replaceExpr s4)
            ...
            | Expr.IfThenElse (a,b,c,d) -> Expr.IfThenElse (replaceExpr a,replaceExpr b,c |> Option.map replaceExpr,d)
            ...
            | Expr.ForEach (b,c,d,e,f) -> Expr.ForEach (b,c,d, replaceExpr e, replaceExpr f)    
    ...
    let rec getFirstExpr f tree = 
        ...
        | None -> 
            match tree with 
            | Expr.Sequential (s2,s3,s4) -> getFirstExpr s3 |> Option.orElse (getFirstExpr s4)
            ...
            | Expr.IfThenElse (a,b,c,d) -> getFirstExpr a |> Option.orElse (getFirstExpr b) |> Option.orElse (c |> Option.bind getFirstExpr)
            | Expr.LetOrUse (x,y,z,i) -> getFirstExpr i
            | Expr.LetOrUseBang (b,c,d,e,f) -> getFirstExpr e |> Option.orElse (getFirstExpr f)
            | Expr.App (a,b,c,d) -> getFirstExpr c |> Option.orElse (getFirstExpr d)
            | Expr.ForEach (b,c,d,e,f) -> getFirstExpr e |> Option.orElse (getFirstExpr f)
    ...
    let rec containsExpr f tree = 
        ...
        match tree with 
        | x when f x -> [x]
        | Expr.Sequential (s2,s3,s4) -> containsExpr s3 @ containsExpr s4
        ...
        | Expr.IfThenElse (a,b,c,d) -> (containsExpr a) @ (containsExpr b)
        | Expr.LetOrUse (x,y,z,i) -> 
            (containsExpr i) @ (z |> List.collect (fun (LetBind (a,b,c,d,e,f,g,h)) -> containsExpr h))
        | Expr.LetOrUseBang (b,c,d,e,f) -> containsExpr e @ containsExpr f
        | Expr.App (a,b,c,d) ->  containsExpr c @ containsExpr d
        | Expr.ForEach (b,c,d,e,f) -> containsExpr e @ containsExpr f
        | Expr.Match (b,c) -> 
    ...
    let replaceAnyPostOrPreIncrement =
        replaceExpr 
            (function
                | Expr.Sequential(b,Expr.LongIdentSet (c,d), expr) -> Expr.LongIdentSet (c,d) |> Some
                | Expr.Sequential(b, expr, Expr.LongIdentSet (c,d)) -> Expr.LongIdentSet (c,d) |> Some
                | _ -> None)
    ...
    let sequential xs = 
        ...
        | [x] -> x
        | xs ->
            // List is built in reverse order to preserve the structure allowing for transformations     
             xs |> List.rev |> Seq.reduce (fun y x ->  // List is rev, y x are reversed to correct this. 
                ...
                | Expr.LetOrUseBang _ -> 
                    replaceExpr (fun tree -> 
                        match tree with 
                        | Expr.LetOrUseBang (b,c,d,e,Expr.InLetPlaceholder) -> Expr.LetOrUseBang (b,c,d,e,y) |> Some
                        | _ -> None) x
                
                | _ -> Expr.Sequential (true, x, y) ) 
    ...
    let createErrorCode location (e:SyntaxNode) = 
    ...
    let (|IsPostFixIncrement|IsPostFixDecrement|IsNotPostFix|) (x:SyntaxNode) = 
        match x with 
        | :? PostfixUnaryExpressionSyntax as x -> 
            match x.OperatorToken with 
            | IsIncrement -> IsPostFixIncrement
            | IsDecrement -> IsPostFixDecrement
            | Neither -> IsNotPostFix
        | _ -> IsNotPostFix

    let (|IsPreFixIncrement|IsPreFixDecrement|IsNotPreFix|) (x:SyntaxNode) = 
        match x with 
        | :? PrefixUnaryExpressionSyntax as x -> 
            match x.OperatorToken with 
            | IsIncrement -> IsPreFixIncrement
            | IsDecrement -> IsPreFixDecrement
            | Neither -> IsNotPreFix
        | _ -> IsNotPreFix
...
module SynType = 
    let rec getName = function 
        ...
        | SynType.App (a,b,c,d,e,f,g) -> getName a
        | x ->
